...
def generate_field(field: pd.Series, file: Path) -> str:
    params = []
    ...
    if is_null:
        ...
        res = combine_param(res=res, param_str=null_str)
    ...
    if col_default is not None and col_default != 'CURRENT_TIMESTAMP':  # only set int/float/str
        ...
        res = combine_param(res=res, param_str=default_str)
    ...
    if col_key:  # cannot handle foreignkey
        if col_key == 'UNI':
            ...
            res = combine_param(res=res, param_str=unique_str)

        elif col_key == 'PRI':
            params.append(col_key)
            pk_str = f"primary_key=True,"
            res = combine_param(res=res, param_str=pk_str)
    ...
    try:
        ...
        if col_str_len > 0:
            ...
            res = combine_param(res=res, param_str=len_str)
    except ValueError:
        pass
...
def make_orm_field(col: pd.Series, file: Path):
    """"""
    field = generate_field(col, file=file)
    with file.open(mode='a', encoding='utf8') as fa:
        fa.write(field + '\n')


def make_orm_table(table, file: Path):
    tb_name: str = table['TABLE_NAME'].values[0]
    rows = (
        f"class {count_model_name(tb_name=tb_name)}(orm.Model):",
    ...
    with file.open(mode='a', encoding='utf8') as fa:
        for row in rows:
            fa.write(row + '\n')
    table.apply(make_orm_field, axis=1, file=file)
    with file.open(mode='a', encoding='utf8') as fa:
        fa.write('\n\n')
